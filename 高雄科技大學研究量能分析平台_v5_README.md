...
- 合作分析摘要已改為符合 flow 邏輯的桑基圖：目前情境文字顯示於放大的 `合作類型` 篩選器旁，桑基圖本體流向為 `來源類型 → 整體 Top 5 來源項目`，來源類型保留 Institution、Country/Region 與 Source，不納入 Author；節點高度與流帶寬度皆使用同一口徑的 `Total` 數值。學科領域與 Keyphrases 不再硬接入桑基圖，而是以 Top 5 摘要卡呈現；桑基圖會跟隨 General 上方 `主要學校` 篩選與合作類型同步重畫，並已拉大圖面高度、節點寬度與群組間距，文字置入深色節點內以提升可讀性。
