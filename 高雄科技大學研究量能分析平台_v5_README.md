...
## 視覺調整
- 背景已改為深色研究漂浮字樣風格，文字改為全英文並自導覽按鈕下方開始散落，中央閱讀區保留乾淨空間；目前增加更多研究治理、資料、專利、永續與 AI 相關字詞，並更分散地配置於上中下與左右外圍背景。背景字樣具備慢速漂浮、滑鼠視差、近距離跟隨、頁面捲動視差與偶發微亮效果，且位置已縮回瀏覽器可視範圍內，讓背景在不干擾閱讀的前提下保有動態感。
...
## 修改紀錄
- 合作分析摘要桑基圖調整配色為 Subject Area 橘色、Keyphrases 綠色、Collaboration Source 紅色；SVG hover 改用網站統一浮窗，Subject Area 類別可顯示完整 subject area 清單，個別 subject area 顯示 Output 與 FWCI。
