...
- 合作分析摘要已改為符合 flow 邏輯的桑基圖：目前情境文字顯示於放大的 `合作類型` 篩選器旁，桑基圖本體流向為 `Top 5 學科領域 / Top 5 Keyphrases / Top 5 合作來源 → 各自 Top 5 項目`，合作來源不納入 Author；由於三類指標單位不同，桑基圖採各類內相對寬度呈現，節點文字會保留原始數值口徑。合作 KPI 會跟隨 General 上方 `主要學校`、`年度` 與合作類型同步重畫；單一年份 KPI 優先使用 General 年度資料，Top 5 明細則使用目前 Collaboration detailed Summary 檔案內容。
