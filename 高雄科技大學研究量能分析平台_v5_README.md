...
- 合作分析摘要已改為符合 flow 邏輯的三層桑基圖：左側為目前篩選情境，流向中間 `Subject Area`、`Keyphrases`、`Collaboration Source` 三類，再流向右側各類 Top 5 項目；合作來源不納入 Author。由於三類指標單位不同，桑基圖採各類內相對寬度呈現，原始數值口徑置入桑基圖節點與流帶的滑鼠提示，包含 Subcategory、Output、FWCI、Keyphrases Relevance 與合作來源 Total。桑基圖標題支援 hover 說明，補充指標與計算方式。合作 KPI 會跟隨 General 上方 `主要學校`、`年度` 與合作類型同步重畫；單一年份 KPI 優先使用 General 年度資料，Top 5 明細則使用目前 Collaboration detailed Summary 檔案內容。
...
## 視覺調整
- 背景已改為深色研究文字雲風格，文字雲改為全英文並自導覽按鈕下方開始散落，中央閱讀區保留乾淨空間；目前增加更多研究治理、資料、專利、永續與 AI 相關字詞，並更分散地配置於上中下與左右外圍背景。文字雲具備慢速漂浮、滑鼠視差、近距離跟隨與頁面捲動視差互動，且位置已縮回瀏覽器可視範圍內，讓背景在不干擾閱讀的前提下保有動態感。
